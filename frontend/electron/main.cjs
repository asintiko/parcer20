...
function resolveClientAccessConfigPath() {
    ...
    if (portableDir) {
        return {
            path: path.join(portableDir, 'security', 'client-access.json'),
            source: 'portable',
        };
    }
    ...
    if (process.platform === 'win32') {
        ...
        return {
            path: path.join(programData, 'TBSparcer', 'security', 'client-access.json'),
            source: 'programdata',
        };
    }
    ...
    return {
        path: path.join(app.getPath('userData'), 'security', 'client-access.json'),
        source: 'userData',
    };
}

function loadClientAccessConfig() {
    const resolved = resolveClientAccessConfigPath();
