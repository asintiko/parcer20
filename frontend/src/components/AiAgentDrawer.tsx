...
import { AiAgentRoutinesPanel } from './AiAgentRoutinesPanel';
import { AiAgentNotificationsPanel } from './AiAgentNotificationsPanel';
...
export const AiAgentDrawer: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const onKey = (e: KeyboardEvent) => {
            if (e.key === 'Escape') {
                // Let an open modal (rename/purge in drawer, delete-confirm in routines
                // panel) handle Escape itself — don't also collapse the pane/drawer.
                if (renameTarget || purgeTarget || routinesModalOpen) return;
                e.preventDefault();
                if (notificationsOpen) {
                    setNotificationsOpen(false);
                } else if (routinesOpen) {
    ...
    }, [closeAgent, isOpen, sidebarOpen, routinesOpen, notificationsOpen, routinesModalOpen, renameTarget, purgeTarget]);
                    ...
                    >
                        ...
                        <div className="agent-pane-viewport">
                            <motion.div
                                ...
                                aria-hidden={routinesOpen}
                                {...({ inert: routinesOpen ? '' : undefined } as Record<string, unknown>)}
                                transition={{
                                    type: 'spring',
                                    stiffness: 380,
                                    damping: 38,
                            ...
                            <AnimatePresence>
                                {routinesOpen && isAdmin ? (
                                    <AiAgentRoutinesPanel
                                        key="routines"
                                        onClose={() => setRoutinesOpen(false)}
                                        onModalOpenChange={setRoutinesModalOpen}
                                    />
                                ) : null}
                            </AnimatePresence>
                            <AnimatePresence>
                                {notificationsOpen ? (
                                    <AiAgentNotificationsPanel
                                        key="notifications"
                                        notifications={notifications.notifications}
                                        reports={notifications.reports}
                                        onClose={() => setNotificationsOpen(false)}
                                        onRead={(id) => notifications.readMutation.mutate(id)}
                                        onReadAll={() => notifications.readAllMutation.mutate()}
                                        onClaim={(itemId) => notifications.claimMutation.mutate(itemId)}
                                        onRelease={(itemId) => notifications.releaseMutation.mutate(itemId)}
                                        onOpenReport={(reportId) => {
                                            void handleOpenReport(reportId);
                                            setNotificationsOpen(false);
                                        }}
                                    />
                                ) : null}
