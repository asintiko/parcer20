...
const QUICK_TILES: Array<{
...
}> = [
    {
        ...
        eyebrow: '01',
        title: 'Сопоставление',
        desc: 'AI подберёт приложения для непривязанных операций — предложения уйдут на проверку.',
        prompt: 'Подбери приложения для непривязанных операций и покажи, сколько предложений на проверку',
        tool: 'app_mapping',
    },
    {
        id: 'verify',
        eyebrow: '02',
        title: 'Проверка',
        desc: 'AI перепроверит распознанные транзакции и подготовит исправления полей.',
        prompt: 'Проверь распознанные транзакции и подготовь исправления',
    ...
    },
    {
        ...
        eyebrow: '03',
        title: 'Сверка',
        desc: 'Сравнит Telegram-историю с базой: что совпало, что пропало, что не разобралось.',
        prompt: 'Сверь Telegram-историю с базой за последнюю неделю',
    ...
    },
    {
        ...
        desc: 'Соберёт итоги недели с разбивкой по источникам и приложениям.',
        prompt: 'Собери недельный отчёт по системе',
        tool: 'report_builder',
    },
];
...
export const AiAgentChat: React.FC<AiAgentChatProps> = ({
    ...
    // Typing "/" at the start (with no space yet) opens the palette and filters by the rest.
    const slashActive = draft.startsWith('/') && !draft.includes(' ');
    const slashFilter = slashActive ? draft.slice(1) : '';

    useEffect(() => {
        if (slashActive) setPaletteOpen(true);
    }, [slashActive]);
    ...
    const handlePickCommand = (cmd: { prompt_template: string; requested_tool?: string | null }) => {
        setPaletteOpen(false);
        if (slashActive) setDraft('');
        void onSend({ text: cmd.prompt_template, requestedTool: cmd.requested_tool ?? null }).catch(() => undefined);
    };
            ...
            <div className="agent-composer">
                <div className="agent-composer-anchor">
                    <AiAgentCommandPalette
                        ...
                        isAdmin={isAdmin}
                        filter={slashActive ? slashFilter : ''}
                    />
                    <form
                    ...
                    >
                        <button
                            type="button"
                            className="agent-composer-cmd-btn"
