                ...
                <div className="flex flex-wrap gap-1.5 items-center">
                    ...
                    <div className="flex items-center gap-1.5 ml-auto">
                        ...
                        <button
                            ...
                            title="Перейти к хвосту текущего вида без смены сортировки"
                        >
                            К последним
                        </button>
                        ...
                        <button
                            ...
                            title="Показать операции от старых к новым"
                        >
                            Сначала старые
                        </button>
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs px-2 py-1.5 bg-surface-2 border border-border rounded-md">
                    ...
                    {(syncState?.freshness?.serverLatestTransactionAt || syncState?.freshness?.localLatestTransactionAt) && (
                        <div className="flex flex-wrap items-center gap-2 text-foreground-secondary">
                            <span>
                                Сервер:{' '}
                                {syncState?.freshness?.serverLatestTransactionAt
                                    ? `записи до ${new Date(syncState.freshness.serverLatestTransactionAt).toLocaleString()}`
                                    : 'записей нет'}
                            </span>
                            <span>
                                Локально:{' '}
                                {syncState?.freshness?.localLatestTransactionAt
                                    ? `записи до ${new Date(syncState.freshness.localLatestTransactionAt).toLocaleString()}`
                                    : 'записей нет'}
                            </span>
                            <span className={syncState?.freshness?.desktopLagging ? 'text-warning' : 'text-success'}>
                                {syncState?.freshness?.desktopLagging ? 'Desktop sync отстаёт' : 'Desktop sync актуален'}
                            </span>
                        </div>
                    )}
