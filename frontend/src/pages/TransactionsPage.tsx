...
export function TransactionsPage() {
    ...
    const [initialScrollToken, setInitialScrollToken] = useState(0);
    ...
    useEffect(() => {
        ...
        if (target.suggested_filters && typeof target.suggested_filters === 'object') {
            setFilters((prev) => normalizeFiltersStateForComparison({
                ...prev,
                search: typeof target.suggested_filters.search === 'string' ? target.suggested_filters.search : prev.search,
                dateFrom: typeof target.suggested_filters.dateFrom === 'string' ? target.suggested_filters.dateFrom : prev.dateFrom,
                dateTo: typeof target.suggested_filters.dateTo === 'string' ? target.suggested_filters.dateTo : prev.dateTo,
                apps: Array.isArray(target.suggested_filters.apps) ? target.suggested_filters.apps : prev.apps,
                source_channel: target.suggested_filters.source_channel || prev.source_channel,
                source_chat_ids: Array.isArray(target.suggested_filters.source_chat_ids)
                    ? target.suggested_filters.source_chat_ids.map((value: any) => Number(value)).filter((value: number) => Number.isFinite(value))
                    : prev.source_chat_ids,
                transaction_types: Array.isArray(target.suggested_filters.transaction_types)
                    ? target.suggested_filters.transaction_types
                    : prev.transaction_types,
                currency: target.suggested_filters.currency || prev.currency,
            }));
        }
    ...
    // Always go to the last page once initial data is available (server/fallback).
    useEffect(() => {
        if (isLockedView || effectiveTotal <= 0 || initialLoadDone) return;
        if (hasScopedPageRestoreRef.current) {
        ...
        setInitialLoadDone(true);
        setInitialScrollToken((prev) => prev + 1);
    }, [isLockedView, effectiveTotal, pageSize, initialLoadDone, page]);
        ...
        <div className="h-full flex flex-col bg-bg">
            <div className="flex-1 min-h-0 overflow-hidden p-4 flex flex-col">
                {!hasSecurityStatusSnapshot && (
                    <div className="mb-3 p-4 bg-warning-light border border-warning/40 rounded-lg">
                        <div className="text-sm text-warning mb-1">
                            Проверяем доступ... Пожалуйста, подождите.
                        </div>
                        {securityStatusQuery.isError && (
                            <div className="text-xs text-danger">
                                {securityStatusErrorText}
                            </div>
                        )}
                    </div>
                )}
                ...
                {(needsInitialUnlock || yearRequiresUnlock) && (
                    <div className="mb-3 p-4 bg-warning-light border border-warning/40 rounded-lg">
                        <div className="text-sm text-warning mb-2">
                            Требуется OTP-код для доступа к {selectedYear ? `папке ${selectedYear}` : 'данным'}.
                        </div>
                        ...
                        <div className="flex items-center gap-2 max-w-xl">
                            <input
                                ...
                                className="flex-1 px-3 py-2 border border-border rounded-md bg-input-bg text-input-text"
                            />
                            <button
                                onClick={() => {
                        ...
                        {otpSecondsLeft > 0 && (
                            <div className="mt-2 text-xs text-foreground-secondary">
                                Код действует еще {otpSecondsLeft} сек
                            </div>
                        )}
                    </div>
                )}
                ...
                {backendFallbackReadOnly && (
                    <div className="mb-3 p-3 bg-warning-light border border-warning/40 rounded-lg text-sm text-warning">
                        Сервер недоступен, включен read-only fallback из локального кеша.
                    </div>
                )}
                {showFilteredOutBanner && (
                    <div className="mb-3 p-3 bg-warning-light border border-warning/40 rounded-lg text-sm text-warning flex items-center justify-between gap-3">
                        <span>Данные скрыты текущими фильтрами или видом. Сбросьте сохранённое состояние этого пользователя.</span>
                        <button
                            type="button"
                            onClick={resetCurrentScopeViewState}
                            className="px-3 py-1.5 rounded-md border border-warning text-warning hover:bg-warning/10 whitespace-nowrap"
                        >
                            Сбросить фильтры и вид
                        </button>
                    </div>
                )}

                <div className="flex-1 min-h-0 flex flex-col bg-surface border border-table-border rounded-lg shadow-sm">
                    <div className="flex-1 min-h-0 overflow-hidden p-0 flex flex-col">
                        <TransactionTable
                            ...
                            scrollToLastRowToken={initialScrollToken}
