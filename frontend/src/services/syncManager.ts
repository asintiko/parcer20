...
export class SyncManager {
    ...
    private shouldSyncTable(table: string, manifest: SyncManifestResponse, localMeta?: SyncMetaEntry): boolean {
        ...
        if (localMeta.checksum !== server.checksum) {
            return true;
        }
        ...
        const serverLatestTxMs = parseIsoMs(server.latest_transaction_at);
        const localLatestTxMs = parseIsoMs(localMeta.latest_transaction_at);
        if (serverLatestTxMs !== null) {
            if (localLatestTxMs === null || localLatestTxMs < serverLatestTxMs) {
                return true;
            }
        }

        const serverLatestRowId = Number(server.latest_row_id || 0);
        const localLatestRowId = Number(localMeta.latest_row_id || localMeta.last_server_id || 0);
        if (serverLatestRowId > 0 && localLatestRowId < serverLatestRowId) {
            return true;
        }
        return false;
    }
    ...
    private async applyTableRows(
        ...
        await db.transaction('rw', tableRef, db.syncMeta, async () => {
            ...
            const latestTransactionAt = tableName === 'transactions'
                ? rows.reduce<string | undefined>((acc, row: any) => {
                    const candidate = typeof row?.transaction_date === 'string' ? row.transaction_date : undefined;
                    if (!candidate) return acc;
                    if (!acc) return candidate;
                    return (parseIsoMs(candidate) || 0) > (parseIsoMs(acc) || 0) ? candidate : acc;
                }, existingMeta?.latest_transaction_at)
                : existingMeta?.latest_transaction_at;
            await db.syncMeta.put({
                ...
                latest_row_id: typeof lastServerId === 'number' ? lastServerId : existingMeta?.latest_row_id,
            });
        });
    }
