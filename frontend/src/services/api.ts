...
export const apiClient = axios.create({
    ...
    withCredentials: !isElectronRuntime,
    headers: {
...
apiClient.interceptors.request.use((config) => {
    const applyAuthHeaders = async () => {
        config.withCredentials = !isElectronRuntime;
        if (isElectronRuntime) {
            const access = await loadElectronSystemAccessStatus();
            if (!access || !access.ok) {
                throw new Error(
...
export interface TransactionListResponse {
    ...
    oldest_transaction_at?: string | null;
    server_generated_at?: string | null;
}
...
export interface TelegramChatMessage {
    ...
    // Server-enriched receipt processing status
    processed?: boolean | null;
    processed_status?: 'done' | 'queued' | 'processing' | 'failed' | 'not_processed' | null;
    transaction_id?: number | null;
...
export interface TelegramChat {
    ...
    backlog_lag_messages?: number | null;
    backlog_state?: 'healthy' | 'lagging' | 'stalled' | 'error' | null;
}
...
export const periodsApi = {
    ...
    create: async (payload: { date_from: string; date_to: string; reason?: string; excluded_user_ids?: number[] }): Promise<PeriodRecord> => {
        const response = await apiClient.post<PeriodRecord>('/api/periods', payload);
        return response.data;
    },
    update: async (
        id: number,
        payload: Partial<{ date_from: string; date_to: string; reason: string; is_active: boolean; excluded_user_ids: number[] }>
...
export type RoutineKind = 'reconcile' | 'summary' | 'receipt_audit' | 'custom';
...
export const routinesApi = {
    list: async (): Promise<Routine[]> => {
        const response = await apiClient.get<{ routines: Routine[] }>('/api/routines');
        return response.data.routines || [];
    },
    create: async (payload: RoutineCreate): Promise<Routine> => {
        const response = await apiClient.post<Routine>('/api/routines', payload);
        return response.data;
    },
    update: async (id: string, payload: Partial<RoutineCreate>): Promise<Routine> => {
        const response = await apiClient.patch<Routine>(`/api/routines/${id}`, payload);
        return response.data;
    },
    remove: async (id: string): Promise<void> => {
        await apiClient.delete(`/api/routines/${id}`);
    },
    run: async (id: string): Promise<{ ok: boolean; status: string }> => {
        const response = await apiClient.post<{ ok: boolean; status: string }>(`/api/routines/${id}/run`);
        return response.data;
    },
    runs: async (id: string, limit = 50): Promise<RoutineRun[]> => {
        const response = await apiClient.get<{ runs: RoutineRun[] }>(`/api/routines/${id}/runs`, {
            params: { limit },
        });
...
export interface AgentThread {
    ...
    latest_run_tool_name?: string | null;
    latest_run_updated_at?: string | null;
    latest_run_requires_confirmation?: boolean;
...
export interface AgentMessageContent extends Record<string, any> {
    message?: string;
    cards?: any[];
    navigation_targets?: AgentLocateResult[];
    ui_action?: AgentUiActionPayload;
}

export interface AgentUiActionPayload {
    kind:
        | 'apply_filters'
        | 'clear_filters'
        | 'mark_rows'
...
export interface AgentRunEvent {
    id: string;
    run_id: string;
    event_type: 'planning_started' | 'tool_selected' | 'tool_started' | 'tool_progress' | 'tool_finished' | 'awaiting_confirmation' | 'completed' | 'failed';
    label: string;
...
export interface AgentThreadDetail extends AgentThread {
    ...
    runs: AgentRun[];
    run_events: AgentRunEvent[];
}

export interface AgentSuggestedFilters {
    search?: string | null;
...
export interface AgentLocateResult {
    ...
    focus_mode: 'row' | 'filter' | string;
    suggested_filters: AgentSuggestedFilters;
    sort_hint: Record<string, any>;
    page_hint?: number | null;
    highlight_seconds?: number | null;
    cursor_hint?: Record<string, any> | null;
}

export const agentApi = {
    listThreads: async (params?: { view?: 'active' | 'archived' | 'trash'; search?: string }): Promise<AgentThread[]> => {
        const response = await apiClient.get<AgentThread[]>('/api/agent/threads', { params });
        return response.data;
    },
    ...
    updateThread: async (threadId: string, payload: { title?: string; context?: Record<string, any> }): Promise<AgentThread> => {
        const response = await apiClient.patch<AgentThread>(`/api/agent/threads/${threadId}`, payload);
        return response.data;
    },
    archiveThread: async (threadId: string): Promise<AgentThread> => {
        const response = await apiClient.post<AgentThread>(`/api/agent/threads/${threadId}/archive`);
        return response.data;
    },
    trashThread: async (threadId: string): Promise<AgentThread> => {
        const response = await apiClient.post<AgentThread>(`/api/agent/threads/${threadId}/trash`);
        return response.data;
    },
    restoreThread: async (threadId: string): Promise<AgentThread> => {
        const response = await apiClient.post<AgentThread>(`/api/agent/threads/${threadId}/restore`);
        return response.data;
    },
    purgeThread: async (threadId: string): Promise<{ ok: boolean; thread_id: string }> => {
        const response = await apiClient.post<{ ok: boolean; thread_id: string }>(`/api/agent/threads/${threadId}/purge`);
        return response.data;
    },
    ...
    rejectRun: async (runId: string): Promise<AgentRun> => {
        const response = await apiClient.post<AgentRun>(`/api/agent/runs/${runId}/reject`);
        return response.data;
    },
...
export interface ChatCommandCreate {
    title: string;
    prompt_template: string;
    icon?: string | null;
    requested_tool?: string | null;
...
export const chatCommandsApi = {
    listCommands: async (): Promise<ChatCommand[]> => {
        const response = await apiClient.get<ChatCommand[] | { commands: ChatCommand[] }>('/api/agent/commands');
        const data = response.data as any;
        return data?.commands ?? data ?? [];
    },
    createCommand: async (payload: ChatCommandCreate): Promise<ChatCommand> => {
        const response = await apiClient.post<ChatCommand>('/api/agent/commands', payload);
        return response.data;
    },
    updateCommand: async (id: string, payload: Partial<ChatCommandCreate>): Promise<ChatCommand> => {
        const response = await apiClient.patch<ChatCommand>(`/api/agent/commands/${id}`, payload);
        return response.data;
    },
    deleteCommand: async (id: string): Promise<void> => {
        await apiClient.delete(`/api/agent/commands/${id}`);
    },
};
