...
- Windows-клиент возвращён на совместимый с предыдущими версиями unsigned-канал
  автообновлений по явному решению владельца продукта; целостность скачивания
  продолжает проверяться по SHA-512 из `latest.yml`.
- Исправлена новая блокировка updater: проверка обновлений снова работает без
  `publisherName` и Authenticode-сертификата, включая проверку сразу после запуска.
