 ...
 *
 * DESIGN-SANDBOX MODE: replaced live axios with an in-memory mock client.
 * Real network calls are short-circuited; admin/admin logs in instantly.
 */
...
export const apiClient: any = createMockApiClient();
...
const loadBrowserSystemAccessStatus = async (_force = false): Promise<ElectronSystemAccessStatus | null> => {
    // Design sandbox: bypass /client-access.json gate. Pretend access is granted.
    if (typeof window === 'undefined' || isElectronRuntime) {
        return null;
    }
    const stub: ElectronSystemAccessStatus = {
        ok: true,
        token: 'design-sandbox',
        apiBaseUrl: null,
        source: 'design-sandbox',
    };
    setPersistentWithLegacyCleanup(SYSTEM_ACCESS_TOKEN_KEY, 'design-sandbox');
    browserSystemAccessStatusPromise = Promise.resolve(stub);
...
export const telegramClientApi = {
    ...
    getProcessStatus: async (chatId: number, messageId: number): Promise<TelegramProcessResult> => {
        const response = await apiClient.get<TelegramProcessResult>(
            `/api/tg/chats/${chatId}/messages/${messageId}/process-status`,
            { headers: chatAccessHeaders(chatId) }
        );
        return response.data;
    },
