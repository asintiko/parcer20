...
export const UserbotPage: React.FC = () => {
    ...
    const allMessages: TelegramChatMessage[] = messagesQuery.data?.items || [];
    ...
    // Process message — kicks off the queued→processing→done/failed flow
    // and polls /process-status until terminal. Returns a Promise<'done' | 'failed'>.
    const runProcess = useCallback(
        async (chatId: number, messageId: number): Promise<'done' | 'failed'> => {
            ...
            try {
                let res = await telegramClientApi.processMessage(chatId, messageId);
                let attempts = 0;
                const maxAttempts = 30; // ~9s ceiling
                while (res.status !== 'done' && res.status !== 'failed' && attempts < maxAttempts) {
                    await new Promise((resolve) => setTimeout(resolve, 300));
                    res = await telegramClientApi.getProcessStatus(chatId, messageId);
                    attempts += 1;
                }
                setProcessingIds((s) => {
                    const n = new Set(s);
