...
# Backend internal endpoint for the receipt "reprocess" button. The bot has no
# TDLib session, so it delegates reprocessing to the backend container.
RECEIPT_REPROCESS_URL = os.getenv("BACKEND_INTERNAL_URL", "http://backend:8000").rstrip("/") + "/api/internal/receipt/reprocess"
...
async def _cb_reprocess_receipt(callback: CallbackQuery, data: str) -> None:
    """Handle the 'reprocess' button on a receipt-log failure message.

    callback_data: `rcpt:repro:{chat_id}:{message_id}`. Delegates to the backend
    internal endpoint (which owns the TDLib session). On success the backend
    posts a fresh processed-receipt message; here we just edit the failure
    message to mark it resolved (and drop the button). On repeated failure we
    ...
    try:
        ...
        with suppress(Exception):
            await callback.answer("Некорректный chat/message id", show_alert=True)
        return

    if not INTERNAL_API_KEY:
        with suppress(Exception):
            await callback.answer("INTERNAL_API_KEY не настроен", show_alert=True)
        return

    with suppress(Exception):
        await callback.answer("Повторная обработка запущена…")
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(60.0, connect=5.0)) as client:
            resp = await client.post(
                RECEIPT_REPROCESS_URL,
                json={"chat_id": chat_id, "message_id": message_id},
                headers={"X-Internal-Api-Key": INTERNAL_API_KEY},
            )
        if resp.status_code < 400:
            body = resp.json()
            result_status = str(body.get("status") or "failed")
    ...
    if result_status in ("processed", "duplicate"):
        # Success: drop the button, mark the failure message resolved. The
        # backend already posted a fresh processed/duplicate message.
        label = "обработан повторно ✅" if result_status == "processed" else "уже в базе (дубликат) ♻️"
        try:
            base = msg.html_text if getattr(msg, "html_text", None) else (msg.text or "")
            await msg.edit_text(f"{base}\n\n— Чек {label}", reply_markup=None)
        except TelegramBadRequest:
            with suppress(Exception):
                await msg.edit_reply_markup(reply_markup=None)
    else:
        # Still failing: keep the button for another attempt; surface the reason.
        with suppress(Exception):
            await callback.answer(f"Не удалось: {error_text or 'ошибка'}"[:200], show_alert=True)
...
async def _handle_callback(callback: CallbackQuery) -> None:
    ...
    if data.startswith("rcpt:repro:"):
        await _cb_reprocess_receipt(callback, data)
        return
