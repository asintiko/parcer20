...
def _normalize_direction(direction: Optional[str], query_text: str) -> str:
    ...
    if raw in {"spent", "received", "turnover", "any"}:
        return raw
    return "any"
...
def _build_transactions_navigation_target(
    *,
    ...
    if apps:
        merged_apps.extend([str(item).strip() for item in apps if str(item).strip()])
    if application and application not in merged_apps:
        merged_apps.append(application)
    if merged_apps:
        suggested_filters["apps"] = merged_apps
    if operators:
        cleaned_ops = [str(op).strip() for op in operators if str(op).strip()]
        if cleaned_ops:
            suggested_filters["operators"] = cleaned_ops
...
async def transaction_analytics(db: Session, current_user: Dict[str, Any], scope: Optional[dict], args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    apps_arg = args.get("apps")
    apps_list: Optional[List[str]] = None
    if isinstance(apps_arg, list):
        apps_list = [str(item).strip() for item in apps_arg if str(item).strip()]
        if not apps_list:
            apps_list = None

    operators_arg = args.get("operators")
    operators_list: Optional[List[str]] = None
    if isinstance(operators_arg, list):
        operators_list = [str(item).strip() for item in operators_arg if str(item).strip()]
    ...
    amount_min = _to_decimal(args.get("amount_min"))
    amount_max = _to_decimal(args.get("amount_max"))
    ...
    if source_channel not in {None, "TELEGRAM", "SMS", "MANUAL", "ALL"}:
        source_channel = None
    ...
    nav_target = _build_transactions_navigation_target(
        start=date_from,
        end=date_to,
        direction=direction,
        currency=currency,
