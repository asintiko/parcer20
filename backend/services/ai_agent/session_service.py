...
# Must mirror the check_agent_run_event_type CHECK constraint in models.py.
...
def serialize_run(row: AgentRun) -> JsonDict:
    result_dict = _load_json(row.result_json) if row.result_json else None
    confirmation_payload = None
    if isinstance(result_dict, dict):
        candidate = result_dict.get("confirmation_payload")
        if isinstance(candidate, dict):
            confirmation_payload = candidate
    return {
        "id": str(row.id),
        "thread_id": str(row.thread_id),
        "created_by_user_id": int(row.created_by_user_id),
...
def create_run_event(
    ...
    *,
    ...
    normalized_type = event_type if event_type in _ALLOWED_RUN_EVENT_TYPES else "failed"
    if normalized_type != event_type:
        logger.warning(
            "Unknown agent_run_event type %r normalized to 'failed' (run_id=%s)",
            event_type,
            run_id,
        )
    row = AgentRunEvent(
    ...
    try:
        ...
        # Telemetry must never poison the run transaction. Roll back so the
        # caller's session stays usable and the run can still be finalized.
        logger.exception("Failed to persist agent_run_event (run_id=%s)", run_id)
        db.rollback()
        return None
    db.refresh(row)
    return row
