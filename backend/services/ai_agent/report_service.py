...
def generate_report(
    ...
    *,
    ...
    failed_parse_count = db.query(ReceiptProcessingTask).filter(ReceiptProcessingTask.status == "failed").count()
    pending_mapping_count = db.query(AutomationSuggestion).filter(AutomationSuggestion.status == "pending").count()
    pending_duplicates_count = db.query(DuplicateSuggestion).filter(DuplicateSuggestion.status == "pending").count()
    pending_reconciliation_count = db.query(ReconciliationSuggestion).filter(ReconciliationSuggestion.status == "pending").count()
