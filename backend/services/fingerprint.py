...
def compute_fingerprint_v2(
    ...
    *,
    ...
    type_part = (str(transaction_type or "")).strip().upper()
    data = f"{amount_str}|{date_str}|{card_str}|{operator_part}|{type_part}"
    return hashlib.sha256(data.encode()).hexdigest()
...
def compute_fingerprint_candidates(
    ...
    *,
    ...
    fp_v2 = compute_fingerprint_v2(
        ...
        transaction_type=transaction_type if include_type else None,
    )
