...
class RegexParser:
    ...
    def parse_date(self, date_str: str, time_str: str, format_type: str = 'standard') -> datetime:
        ...
        try:
            if format_type == 'semicolon':
                ...
                parts = date_str.split('.')
                if len(parts[2]) == 2:
                    # 2-digit year: clamp to current century, but if it would
                    # result in a far-future date relative to "now", treat as
                    # malformed input and reject (avoids 99 → 2099 silent slip).
                    yy = int(parts[2])
                    full_year = 2000 + yy
                    if full_year > datetime.utcnow().year + 1:
                        raise ValueError(f"2-digit year out of range: {yy}")
                    parts[2] = str(full_year)
    ...
    def parse_humo_notification(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        # Extract operator
        operator_match = re.search(patterns['operator'], text, re.IGNORECASE)
        ...
        # Extract datetime
        datetime_match = re.search(patterns['datetime'], text, re.IGNORECASE)
        if datetime_match:
            ...
            datetime_fallback = re.search(patterns['datetime_fallback'], text, re.IGNORECASE)
        ...
        # Extract balance
        balance_match = re.search(patterns['balance'], text, re.IGNORECASE)
        balance_after = self.normalize_amount(balance_match.group(1)) if balance_match else None
        
        # Extract currency
        currency = amount_currency
        if not currency:
            currency_match = re.search(patterns['currency'], text, re.IGNORECASE)
    ...
    def parse_sms_inline(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        Skip when the text is a multi-line Uzum/UZUM Bank receipt — those have
        labelled amounts like "Sender amount 21,978,000.00 UZS" and should be
        handled by parse_sender_receiver_transfer / parse_ru_transfer. The SMS
        regex anchors on a comma and would clip the leading digits of the amount.
        """
        uzum_markers = (
            "sender amount",
            "receiver amount",
        ...
        lower = text.lower()
        if any(marker in lower for marker in uzum_markers):
            return None
        ...
        if not datetime_match:
            datetime_match = re.search(patterns['datetime_iso'], text, re.IGNORECASE)
        ...
        type_match = re.search(patterns['type_keyword'], text, re.IGNORECASE)
        if type_match:
    ...
    def parse_semicolon_format(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        card_amount_match = re.search(patterns['card_amount'], text, re.IGNORECASE)
    ...
    def parse_cardxabar(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        dt_match = re.search(patterns['datetime'], text, re.IGNORECASE)
        if dt_match:
