...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    Also propagates X-Request-ID through contextvars so downstream loggers
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        token = set_request_id(rid)  # noqa: F841 — context is per-request
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    HSTS is gated on `STRICT_TRANSPORT_SECURITY=1` env to avoid breaking dev
    where the app is served over plain HTTP.
    """

    _ENABLE_HSTS = str(os.getenv("STRICT_TRANSPORT_SECURITY", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
class SystemAccessMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        # Spreadsheet feed authenticates via EXPORT_FEED_KEY only.
...
# `null` origin (sandbox iframes, file://) допускается только если задан env-флаг.
# Без флага — отключено для защиты от CSRF из sandbox iframes.
if str(os.getenv("CORS_ALLOW_NULL_ORIGIN", "")).strip().lower() in {"1", "true", "yes", "on"}:
...
async def internal_metrics(
    ...
    """In-process metrics snapshot. Requires X-Internal-Api-Key."""
    from services.internal_api_key_service import is_valid_internal_api_key
    from services import metrics as _metrics
    ...
    try:
        ...
        cum = getattr(provider, "cumulative_usage", None)
        if cum:
            snap["ai_usage_cumulative"] = cum
            snap["ai_last_usage"] = getattr(provider, "last_usage", None)
    except Exception:  # noqa: BLE001
        pass
    return snap
...
app.include_router(telegram_client.files_router, tags=["Telegram"])
app.include_router(telegram_client.ws_router, tags=["Telegram"])
app.include_router(telegram_client.reprocess_router, tags=["TelegramInternal"])
