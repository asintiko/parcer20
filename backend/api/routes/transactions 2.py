...
def _apply_scope_to_query(query, scope: Optional[Dict[str, Any]]):
    ...
    if scope_years:
        # Use range clauses (one per year) to leverage idx_transactions_date.
        year_ranges = []
        for y in scope_years:
            ...
            year_ranges.append(
                and_(
                    Transaction.transaction_date >= datetime(yi, 1, 1),
                    Transaction.transaction_date < datetime(yi + 1, 1, 1),
                )
            )
        if year_ranges:
            query = query.filter(or_(*year_ranges))
    return query
...
def _apply_allowed_sources_to_query(query, current_user: Optional[Dict[str, Any]]):
    ...
    set of chat_ids for a scoped operator. An empty set means the operator is
    bound to zero sources → they must see nothing. This must run unconditionally
    on every read so a missing `source_chat_ids` query param can't widen access.
    """
...
def _ensure_source_allowed(tx: Optional[Transaction], current_user: Optional[Dict[str, Any]]) -> None:
    """Raise 404 if a single transaction is outside the operator's allowed sources.

    404 (not 403) so existence of out-of-scope rows is not disclosed.
    """
    if tx is None:
        return
    allowed = get_allowed_sources_for_user(current_user)
...
def _apply_user_forbidden_ranges_to_query(query, ranges: List[tuple[date, date]]):
    ...
    for range_start, range_end in ranges:
        # Range-based comparison uses idx_transactions_date.
        # Convert dates to inclusive [start_dt, end_dt+1day) interval.
...
def _apply_locked_periods_to_query(query, db: Session):
    ...
    clauses = []
    for lock in locks:
        start_dt = datetime.combine(lock.date_from, datetime.min.time())
        end_dt = datetime.combine(lock.date_to, datetime.min.time()) + timedelta(days=1)
        clauses.append(
            and_(
                Transaction.transaction_date >= start_dt,
                Transaction.transaction_date < end_dt,
...
async def bulk_update_transactions(
    ...
    try:
        ...
        existing_rows = (
            ...
            .filter(Transaction.id.in_(request_ids))
            .all()
        )
        ...
        for item in request.updates:
            ...
            if not is_date_allowed_for_user(current_user, c.transaction_date):
                ...
                continue
            if period_lock_service.is_date_locked(
            ...
            if "card_last_4" in fields:
                c.card_last_4 = fields["card_last_4"] or None
            ...
            if "receiver_card" in fields:
                c.receiver_card = fields["receiver_card"] or None
...
async def create_transaction(
    ...
    try:
        ...
        _assert_not_in_locked_period(db, dt, current_user=current_user)
...
async def get_transaction(
    ...
    _assert_not_in_locked_period(db, tx.transaction_date, current_user=current_user)
...
async def update_transaction(
    ...
    try:
        ...
        _assert_not_in_locked_period(db, c.transaction_date, current_user=current_user)
        ...
        if "transaction_date" in update_dict:
            ...
            _assert_not_in_locked_period(db, update_dict["transaction_date"], current_user=current_user)
        ...
        if "card_last_4" in update_dict:
            c.card_last_4 = update_dict["card_last_4"] or None
        ...
        if "receiver_card" in update_dict:
            c.receiver_card = update_dict["receiver_card"] or None
...
async def delete_transaction(
    ...
    try:
        ...
        _assert_not_in_locked_period(db, transaction.transaction_date, current_user=current_user)
