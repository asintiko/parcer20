...
# Models whose rows are bound to a Telegram source chat. For a scoped operator
# these must be filtered by allowed_sources so the offline mirror cannot leak
...
def _table_manifest(
    ...
    latest_transaction_at = None
    if model is Transaction:
        latest_transaction_at = query.with_entities(func.max(Transaction.transaction_date)).scalar()
    ...
    return {
        ...
        "checksum": checksum,
        "latest_row_id": int(latest_row_id) if latest_row_id is not None else None,
        "latest_transaction_at": latest_transaction_at.isoformat() if isinstance(latest_transaction_at, datetime) else None,
    }
