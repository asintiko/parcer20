...
async def _process_reconciliation(
    ...
    with SessionLocal() as db:
        ...
        # Step 6: Check for failed processing tasks
        failed_lookup: Dict[tuple, str] = {}
        if receipt_candidates:
            for cid, mid in candidate_pairs:
                failed_task = (
                    db.query(ReceiptProcessingTask.status, ReceiptProcessingTask.error)
                    .filter(
                        ReceiptProcessingTask.chat_id == str(cid),
                        ReceiptProcessingTask.message_id == str(mid),
                        ReceiptProcessingTask.status == "failed",
                    )
                    .first()
                )
                if failed_task:
                    failed_lookup[(cid, mid)] = failed_task[1] or "unknown error"
