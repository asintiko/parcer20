...
from api.dependencies import (
    ...
    get_scope_context_optional,
    get_system_access_context,
    require_admin_user,
)
...
from services.ai_agent import chat_command_service as ccs
...
class ChatCommandCreateRequest(BaseModel):
    title: str = Field(..., min_length=1, max_length=120)
    prompt_template: str = Field(..., min_length=1, max_length=4000)
    icon: Optional[str] = Field(default=None, max_length=40)
    requested_tool: Optional[str] = Field(default=None, max_length=80)
    scope: str = Field(default="team", pattern="^(team|personal)$")
    sort_order: int = Field(default=0)
    enabled: bool = True


class ChatCommandUpdateRequest(BaseModel):
    title: Optional[str] = Field(default=None, min_length=1, max_length=120)
    prompt_template: Optional[str] = Field(default=None, min_length=1, max_length=4000)
    icon: Optional[str] = Field(default=None, max_length=40)
    requested_tool: Optional[str] = Field(default=None, max_length=80)
    scope: Optional[str] = Field(default=None, pattern="^(team|personal)$")
...
@router.get("/commands")
async def list_chat_commands_route(
    db: Session = Depends(get_db_session),
    current_user: dict = Depends(get_current_app_user),
):
    user_id = _user_id_from_snapshot(current_user)
    return {"commands": ccs.list_commands(db, user_id=user_id)}


@router.post("/commands")
async def create_chat_command_route(
    payload: ChatCommandCreateRequest,
    db: Session = Depends(get_db_session),
    current_user: dict = Depends(require_admin_user),
    _system: Optional[Dict[str, Any]] = Depends(get_system_access_context),
):
    try:
        row = ccs.create_command(
            db,
            title=payload.title,
    ...
    return ccs.serialize_command(row)


@router.patch("/commands/{command_id}")
async def update_chat_command_route(
    command_id: UUID,
    payload: ChatCommandUpdateRequest,
    db: Session = Depends(get_db_session),
    current_user: dict = Depends(require_admin_user),
    _system: Optional[Dict[str, Any]] = Depends(get_system_access_context),
):
    command = ccs.get_command(db, command_id)
    if command is None:
        raise HTTPException(status_code=404, detail="command_not_found")
    try:
        row = ccs.update_command(db, command, **payload.model_dump(exclude_unset=True))
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    return ccs.serialize_command(row)
...
async def delete_chat_command_route(
    command_id: UUID,
    db: Session = Depends(get_db_session),
    current_user: dict = Depends(require_admin_user),
    _system: Optional[Dict[str, Any]] = Depends(get_system_access_context),
...
async def confirm_agent_run(
    ...
    if row.status in {"cancelled", "completed", "failed"} and not row.requires_confirmation:
        raise HTTPException(status_code=409, detail=f"run_already_{row.status}")
...
async def reject_agent_run(
    ...
    if not row.requires_confirmation:
        if row.status in {"cancelled", "completed", "failed"}:
            raise HTTPException(status_code=409, detail=f"run_already_{row.status}")
        raise HTTPException(status_code=409, detail="run_not_awaiting_confirmation")
    ...
    create_message(
        ...
        role="system",
        message_type="text",
        content={"message": "Действие отклонено."},
    )
    return serialize_run(updated or row)
